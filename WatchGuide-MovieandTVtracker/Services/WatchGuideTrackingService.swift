//
//  WatchGuideTrackingService.swift
//  WatchGuide-MovieandTVtracker
//
//  WatchGuide Tracking Service — Merged from WatchHour. Starts a watch session
//  when the user taps Watch / Continue Watching, prompts once the estimated runtime
//  elapses (Finished / Still Watching / On Hold), and records viewing history,
//  statistics, streaks, ratings, and watch time. Data is persisted via StorageService
//  and synced across devices through the cloud snapshot. Now integrated with TV Time
//  for community insights and global tracking.
//
...
// MARK: - WatchGuide Tracking Service

@MainActor
final class WatchGuideTrackingService: ObservableObject {
    static let shared = WatchGuideTrackingService()
    ...
    nonisolated static let sessionCategoryID = "WATCHGUIDE_TRACKING_SESSION"
    nonisolated static let finishedAction = "WATCHGUIDE_TRACKING_FINISHED"
    nonisolated static let stillWatchingAction = "WATCHGUIDE_TRACKING_STILL_WATCHING"
    nonisolated static let holdAction = "WATCHGUIDE_TRACKING_HOLD"
    ...
    // MARK: - TV Time Integration
    
    /// Get user's contribution to trending data for a specific title
    func getContributionStats(for item: SavedMediaItem) -> (viewCount: Int, avgRating: Double) {
        let history = StorageService.shared.watchHistory
            .filter { $0.mediaId == item.mediaId && $0.mediaType == item.mediaType }
        
        let viewCount = history.count
        let avgRating = history.compactMap { $0.rating }.isEmpty ? 0.0 :
            Double(history.compactMap { $0.rating }.reduce(0, +)) / Double(history.compactMap { $0.rating }.count)
        
        return (viewCount, avgRating)
    }
    
    /// Get top watched titles for community insights
    func getTopWatchedTitles(limit: Int = 10) -> [(title: String, count: Int, avgRating: Double)] {
        let grouped = Dictionary(grouping: StorageService.shared.watchHistory, by: { $0.title })
        
        return grouped
            .map { title, entries in
                let avgRating = entries.compactMap { $0.rating }.isEmpty ? 0.0 :
                    Double(entries.compactMap { $0.rating }.reduce(0, +)) / Double(entries.compactMap { $0.rating }.count)
                return (title: title, count: entries.count, avgRating: avgRating)
            }
    ...
    /// Begins a WatchGuide Tracking session for the given title, recording the runtime,
    ...
    func startSession(
        ...
        let session = WatchSession(
            ...
            deviceName: WatchGuideDevice.currentName,
    ...
    private func scheduleSessionPrompt(for session: WatchSession, afterMinutes: Int? = nil) async {
        ...
        content.userInfo = ["watchGuideTrackingSessionId": session.id]
    ...
    static func notificationId(forSessionId id: String) -> String {
        "watchguide_tracking_session_\(id)"
    }
